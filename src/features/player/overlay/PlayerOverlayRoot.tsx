...
import { AppState, DeviceEventEmitter, Image, Platform, Pressable, StyleSheet, Text, View, useWindowDimensions } from 'react-native';
import Animated, {
    FadeIn,
    FadeOut,
    useAnimatedStyle,
...
export default function PlayerOverlayRoot(props: PlayerOverlayRootProps) {
    ...
    useEffect(() => {
        ...
        const nativeEvents = DeviceEventEmitter.addListener('nativePlayerEvent', (evt: NativePlayerEvent) => {
            ...
            if (type === 'tracks') {
                ...
                setAudioTracks(a.map((t: any) => ({ ...t, title: t.name || t.title || t.language || `Track ${t.id}` })));
                setSubtitleTracks(s.map((t: any) => ({ ...t, title: t.name || t.title || t.language || 'Unknown' })));
                return;
            }
    ...
    const onSelectSpeed = useCallback(
        (rate: number) => {
            ...
            // Native API will be added; keep call guarded for dev.
            (CrispyNativeCore as any).nativePlayerSetRate?.(rate);
            resetControlsTimer();
        },
        [resetControlsTimer]
    );
    ...
    const onSelectResizeMode = useCallback(
        (mode: 'contain' | 'cover' | 'stretch') => {
            setResizeMode(mode);
            (CrispyNativeCore as any).nativePlayerSetResizeMode?.(mode);
            resetControlsTimer();
        },
        [resetControlsTimer]
    );
    ...
    const onUpdateSubtitleDelay = useCallback(
        (delaySec: number) => {
            ...
            (CrispyNativeCore as any).nativePlayerSetSubtitleDelay?.(delaySec);
            resetControlsTimer();
        },
        [resetControlsTimer]
    );
                    ...
                    {activeTab === 'audio' && (
                        <AudioTab
                            ...
                            onSelectTrack={(track) => {
                                ...
                                (CrispyNativeCore as any).nativePlayerSetAudioTrack?.(id);
                                setActiveTab('none');
                            }}
                        />
                    )}

                    {activeTab === 'subtitles' && <SubtitlesTab delay={subtitleDelay} onUpdateDelay={onUpdateSubtitleDelay} />}
